...
#define net_queue_path "/tmp/net_queue"  // 消息队列文件路径


static int net_queue_id = -1; 
static pthread_t net_thread = 0;
...
void http_get_weather_async(char *key,char *city){
    ...
    strcpy(obj.data,"");
    strcpy(obj.type,"GET");
    int ret = systemv_queue_send(net_queue_id, &obj, sizeof(net_obj));
    if(ret == -1)
    {
...
static void* net_thread_fun(void *arg)
{
    ...
    while(1)
    {
        ret = systemv_queue_rcv(net_queue_id, &obj,sizeof(obj) ,IPC_NOWAIT);
        if (ret == -1)
        {
...
int http_request_create()
{
    ...
    ret = systemv_queue_create(net_queue_path,net_queue_id,sizeof(net_obj),50);
    if(ret == -1)
    {
    ...
    ret = pthread_create(&net_thread,NULL,net_thread_fun, NULL);
    if(ret == -1)
    {
